...
positions = pd.read_excel("Traffic Signals Co-ordinates 2021.xlsx")
positions = positions[["FEID", "EASTINGS", "NORTHINGS"]]

positions = positions.rename({'FEID': 'site', 'EASTINGS': 'eastings', 'NORTHINGS': 'northings'}, axis=1)

positions['latitude'] = positions.apply(lambda x: E_N_to_Lat(x['eastings'], x['northings'], a,b,e0,n0,f0,DecimalPHI0,DecimalLAM0), axis=1)
positions['longitude'] = positions.apply(lambda x: E_N_to_Long(x['eastings'], x['northings'], a,b,e0,n0,f0,DecimalPHI0,DecimalLAM0), axis=1)

print(positions)
positions.to_csv('converted_positions.csv')
